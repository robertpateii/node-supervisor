...
var debug = true;
var nodeArgs = [];
var ignore = undefined; // added this for crashWin. maybe there's a better way than adding a global?
...
function run (args) {
  ...
  watchItems.forEach(function (watchItem) {
    ...
    var ignoreDirectory = (ignore == undefined) ? "" : watchItem + '/' + ignore;
    findAllWatchFiles(watchItem, ignoreDirectory, function(f) {
...
function crashWin (event, filename) {
  if ( event === 'change' ) {
    if ( filename != ignore ) {
      ...
      util.debug(filename + ' changed but was ignored.');
    }
  }
}
...
var findAllWatchFiles = function(path, ignoredPath, callback) {
  fs.stat(path, function(err, stats){
    if (err) {
      util.error('Error retrieving stats for file: ' + path);
    } else if (path == ignoredPath) {
      // on Windows, this branch is never run since findAllWatchFiles is not used recursively in windows
      // and ignoredPath has forward slashes inserted in it (win uses backslashes).
      util.debug('Ignoring changes in \'' + path + '\'');
    } else if (isWindows) {
      callback(path);
    } else {
        ...
        fs.readdir(path, function(err, fileNames) {
          if(err) {
            ...
            fileNames.forEach(function (fileName) {
              findAllWatchFiles(path + '/' + fileName, ignoredPath, callback);
            });
          }
        });
      } else {
